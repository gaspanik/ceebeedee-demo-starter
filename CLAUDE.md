# Claude Code ガイドライン（ts-swc）

## プロジェクト概要

React 19 + TypeScript + Vite 8 + Tailwind CSS 4 と TanStack Router を使った最小構成のスターターテンプレートです。
...
- **コンポーネント構造**:
  ...
  - Props はTypeScript インターフェースで明示的に型付け
  - 例：
    ```tsx
...
プロジェクトルートに `DESIGN.md` が存在する場合は、UI の変更を実装する前に必ず読むこと。このファイルにはプロジェクトのデザイントークン（カラー・タイポグラフィ・スペーシング・ボーダー半径）およびコンポーネントガイドラインが含まれている。

## Claude Code 固有のガイドライン

### Tailwind CSS MCP ツール
- **Tailwind ドキュメントには MCP ツールを使用**: 利用可能な場合は Tailwind CSS MCP ツールで最新ドキュメントを参照
  - `mcp__TailwindCSS__get_tailwind_utilities` — カテゴリ・プロパティ・検索ワードでユーティリティを取得
  - `mcp__TailwindCSS__get_tailwind_colors` — カラーパレット情報を取得
  - `mcp__TailwindCSS__search_tailwind_docs` — 公式ドキュメントを検索
  - `mcp__TailwindCSS__convert_css_to_tailwind` — CSS を Tailwind クラスに変換
  - `mcp__TailwindCSS__get_tailwind_config_guide` — フレームワーク固有の設定ガイドを取得
- **v4 構文を検証**: MCP ツールで Tailwind CSS v4 のクラス名とベストプラクティスを必ず確認
- **クラス変換**: 既存の CSS を Tailwind ユーティリティに変換する際は `convert_css_to_tailwind` を使用

### ファイル操作
- ファイルの読み取りには Read ツールを使用
- ファイルの編集には Edit ツールを使用（ターゲットを絞った変更）
- 新規ファイルの作成には Write ツールを使用
- 既存ファイルにはコンテキスト保持のため Write より Edit を優先

### コード検索
- パターンによるファイル検索には Glob を使用（例：`**/*.tsx`）
- ファイル内容の検索には Grep を使用（例：コンポーネントの使用箇所を探す）
- 複雑な調査には subagent_type=Explore で Task ツールを使用

### 変更時のガイドライン
- ファイルを編集する前に必ず内容を読むこと
- 変更はフォーカスを絞って最小限に — 過剰なエンジニアリングを避ける
- 依頼された範囲を超えた機能追加・コメント・リファクタリングは行わない
- 変更後は開発サーバーを起動して動作確認
- コミット前に `<pm> run check` で Biome のリント/フォーマットを確認

### Git 操作
- git コマンドには Bash ツールを使用
- コミット作成時は `git add -A` ではなく特定ファイルをステージング
- 既存のコミットメッセージスタイルに合わせる（`git log` で確認）
- コミットメッセージに `Co-Authored-By: Claude Sonnet 4.5 <noreply@anthropic.com>` を含める

### ベストプラクティス
- ファイルの参照にはマークダウンリンク構文を使用：`[filename.tsx](src/filename.tsx)`
- 特定行の参照：`[filename.tsx:42](src/filename.tsx#L42)`
- 既存のコードスタイルとパターンを常に維持すること
- 明確な必要性がない限り新しい依存関係を追加しないこと
- 回答は簡潔にタスクに集中すること
